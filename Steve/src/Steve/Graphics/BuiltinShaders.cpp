...
namespace Steve
{
	namespace BuiltinShaders
	{
		...
		namespace
		{
		constexpr const char* Renderer2D = R"STEVE_GLSL(
#type vertex
#version 330 core
...
void main()
{
	FragColor = texture(uTextures[int(v_TexIndex)], v_TexCoord) * v_Color;
		...
		constexpr const char* Grid = R"STEVE_GLSL(
#type vertex
#version 330 core
...
void main() {
    ...
    outColor.a *= fading;
		...
		constexpr const char* Phong3D = R"STEVE_GLSL(
#type vertex
#version 450 core
...
void main()
{
	...
	// FragColor = vec4(fs_in.NormalRaw * 0.5 + 0.5, 1.0);
		...
		constexpr const char* Text = R"STEVE_GLSL(
#type vertex
#version 330 core
layout(location = 0) in vec2 a_Position;
layout(location = 1) in vec4 a_Color;
