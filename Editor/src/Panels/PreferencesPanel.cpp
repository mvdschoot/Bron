...
void PreferencesPanel::OnImGuiRender() {
	...
	if (CollapsingHeader("Appearance", ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		restyle |= SliderFloat("UI scale", &prefs.ui_scale, 0.5f, 3.0f, "%.2f");
	...
	if (CollapsingHeader("Viewport navigation", ImGuiTreeNodeFlags_DefaultOpen)) {
		Indent();
		DragFloat("Orbit speed", &prefs.camera_orbit_speed, 0.05f, 0.05f, 10.0f, "%.2f rad/s");
		DragFloat("Zoom speed", &prefs.camera_zoom_speed, 0.01f, 0.01f, 2.0f, "%.2f");
		Unindent();
	}
