...
const BufferLayout& OpenGLVertexBuffer::GetBufferLayout() const { return layout_; }

void OpenGLVertexBuffer::SetBufferLayout(const BufferLayout& layout) { layout_ = layout; }
...
OpenGLIndexBuffer::OpenGLIndexBuffer(u32 count) : count_(0) {
	BR_PROFILE_FUNCTION();
	glCreateBuffers(1, &render_id_);
	glBindBuffer(GL_ARRAY_BUFFER, render_id_);
...
OpenGLIndexBuffer::OpenGLIndexBuffer(u32* indices, u32 count) : count_(count) {
	BR_PROFILE_FUNCTION();
	glCreateBuffers(1, &render_id_);
	glBindBuffer(GL_ARRAY_BUFFER, render_id_);
...
u32 OpenGLIndexBuffer::GetCount() const { return count_; }
...
bool OpenGLUniformBuffer::IsBound() { return bound; }
