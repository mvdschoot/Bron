...
OpenGLTexture2D::OpenGLTexture2D(const void* buffer, u32 size) {
	...
	int width, height, channels;
	stbi_uc* data = stbi_load_from_memory(static_cast<const stbi_uc*>(buffer), static_cast<int>(size), &width, &height,
										  &channels, 0);
...
GLenum OpenGLTexture2D::ToGlEnum(u32 slot) { return GL_TEXTURE0 + slot; }
} // namespace bron
