...
void OpenGLVertexArray::Bind() { glBindVertexArray(renderer_id_); }
...
void OpenGLVertexArray::AddVertexBuffer(Ref<VertexBuffer> buffer) {
	...
	for (const BufferElement& el: layout.GetElements()) {
		...
		glVertexAttribPointer(vertex_buffer_index_, el.GetComponentCount(), ShaderDataTypeToOpenGLType(el.type),
							  el.normalized ? GL_TRUE : GL_FALSE, layout.GetStride(),
...
const std::vector<Ref<VertexBuffer>>& OpenGLVertexArray::GetVertexBuffers() { return vertex_buffers_; }

const Ref<IndexBuffer>& OpenGLVertexArray::GetIndexBuffer() { return index_buffer_; }
} // namespace bron
