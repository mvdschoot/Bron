...
void TextRenderer::Init() {
	if (FT_Init_FreeType(&s_text_data.FT_library))
		BR_CORE_ERROR("Could not initialise FreeText library");

	s_text_data.ShaderId = R2D::AddShader(Shader::Create(builtin_shaders::Source(builtin_shaders::Id::kText)),
...
Font& TextRenderer::LoadUnicodeFont(const char* font_location, unsigned int init_font_height) {
	...
	if (FT_New_Face(s_text_data.FT_library, font_location, 0, &font.FT_font))
		BR_CORE_ERROR("Failed to load font");
	...
	for (unsigned char c = 32; c < 127; c++) {
		...
		Character character = {glm::ivec4(x, 0, g->bitmap.width, g->bitmap.rows),
							   glm::ivec2(g->bitmap_left, g->bitmap_top), g->advance.x};
...
void TextRenderer::RenderText(const char* text, Font& font, float x, float y, float scale, glm::vec4 color) {
	...
	for (const char* c = text; *c != 0; c++) {
		...
		glm::vec4 txt_loc = {
				((float) ch.Location.x) / font.Texture->GetWidth(), ((float) ch.Location.y) / font.Texture->GetHeight(),
				(float) ch.Location.z / font.Texture->GetWidth(), (float) ch.Location.w / font.Texture->GetHeight()};
