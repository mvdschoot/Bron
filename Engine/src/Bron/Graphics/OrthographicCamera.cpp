...
namespace bron
{
	OrthographicCamera::OrthographicCamera(float left, float right, float bottom, float top, float aspect_ratio)
		: AspectRatio(aspect_ratio), view_mat_(glm::mat4(1.0f)), projection_mat_(glm::ortho(left, right, bottom, top, -1.0f, 1.0f)), zoom_level_(1.0f)
	{
	}

	OrthographicCamera::OrthographicCamera(float left, float right, float bottom, float top, float aspect_ratio,
	                                       glm::vec3 position, float rotation)
		: AspectRatio(aspect_ratio), projection_mat_(glm::mat4(1.0f)), view_mat_(glm::ortho(left, right, bottom, top, -1.0f, 1.0f)), position_(position), target_({0.0f, 0.0f, 0.0f}), rotation_(rotation),
		  zoom_level_(1.0f)
	{
		Recalculate();
	}
