...
namespace bron
{
	...
	void BVH::AddObject(Ref<RigidBody> obj)
	{
		InsertPrimitive(root, obj);
	}

	std::vector<Ref<RigidBody>>& BVH::get(Ref<RigidBody>& object) 
	{
	...
	Ref<BvhNode> BVH::find(Ref<RigidBody> to_find) 
	{
		...
		while (current != nullptr) /* Dummy condition. Can just as well be 'true'. */ {
			if (current->primitives.empty()) {
				...
				BR_CORE_ASSERT(std::find(current->primitives.begin(), current->primitives.end(), to_find) != current->primitives.end(), "Cannot find the object");
