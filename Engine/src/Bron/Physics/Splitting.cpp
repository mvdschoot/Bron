...
void SAH::Split(Ref<BvhNode> node) {
	const float split_step_x = (node->box.max.x - node->box.min.x) / (float) (kSplitPerAxis + 1);
	const float split_step_y = (node->box.max.y - node->box.min.y) / (float) (kSplitPerAxis + 1);
	const float split_step_z = (node->box.max.z - node->box.min.z) / (float) (kSplitPerAxis + 1);
...
float SAH::Cost(Ref<BvhNode> node, AABB a, AABB b) {
	...
	return kCostTraverse + (Asa * 2 * l) + (Bsa * 2 * r);
}
} // namespace bron
