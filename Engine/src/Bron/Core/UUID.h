...
public:
	...
	UUID(const UUID* other) { strcpy(value, other->value); }

	UUID(const UUID& other) { strcpy(value, other.value); }

	~UUID() = default;

	bool operator==(UUID& o) const { return strcmp(value, o.value) == 0; }

	bool operator==(const UUID& o) const { return strcmp(value, o.value) == 0; }
