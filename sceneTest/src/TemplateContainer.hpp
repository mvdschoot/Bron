...
namespace Cheets
{
	...
	class TemplateContainer
	{
		public:
			...
			void insert(T& el)
			{
				...
				m_Storage<T>[this].emplace(el.p_Id, el);
			}
			...
			T& get(UUID& uuid)
			{
				return m_Storage<T>[this][uuid];
			}

			template<class T>
			std::unordered_map<UUID, T>& get()
			{
		...
		private:
			template<class T>
			static std::unordered_map<const TemplateContainer*, std::unordered_map<UUID, T>> m_Storage;

			std::unordered_map<std::type_index, std::function<void(TemplateContainer&)>> m_ClearTemplateByIndex;
	...
	template<class T>
	std::unordered_map<const TemplateContainer*, std::unordered_map<UUID, T>> TemplateContainer::m_Storage;
